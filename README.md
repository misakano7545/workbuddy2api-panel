...
- **预编译二进制**：推送 `v*` tag 时 Actions 构建 linux/windows amd64 的 `wb2api`、`signin_bin`、`login`、`credit` 并挂到该 Release。Actions 页手动运行只上传 artifact，不发 Release。Docker 仍由本地 `docker compose build` 构建
