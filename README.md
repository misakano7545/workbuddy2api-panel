...
- **预编译二进制**：推送 `v*` tag 时 Actions 打 `wb2api-panel-<tag>-windows-amd64.zip`（`wb2api.exe`、`config.example.json`、`README.md`）并挂到该 Release。手动运行只上传 artifact。Docker 仍由本地 `docker compose build` 构建
