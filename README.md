...
- **预编译二进制**：推送 `v*` tag 时 Actions 构建 windows/linux amd64 主程序，各打一个 `wb2api-panel-<tag>-<os>-amd64.zip`（`wb2api.exe` 或 `wb2api`，加 `config.example.json`、`README.md`）并挂到该 Release。手动运行只上传 artifact。Docker 仍由本地 `docker compose build` 构建
